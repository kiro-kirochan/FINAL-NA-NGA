...
$id = (int)($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT * FROM patreg WHERE id = ?");
$stmt->execute([$id]);
$pat = $stmt->fetch();
if (!$pat) { $_SESSION['flash'] = ['type'=>'danger','msg'=>'Patient not found.']; header('Location: /hospital/patients/index.php'); exit; }

$form = ['fname'=>$pat['fname'],'lname'=>$pat['lname'],'email'=>$pat['email'],'password'=>'','contact'=>$pat['contact'],'gender'=>$pat['gender']];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!filter_var($form['email'], FILTER_VALIDATE_EMAIL)) $errors['email'] = 'Invalid email format.';
        ...
        $chk = $pdo->prepare("SELECT COUNT(*) FROM patreg WHERE email = ? AND id != ?");
        $chk->execute([$form['email'], $id]);
        if ($chk->fetchColumn()) $errors['email'] = 'Email is already used by another patient.';
    }
    ...
    if (empty($errors)) {
        if ($form['password']) {
            $stmt = $pdo->prepare("UPDATE patreg SET fname=?, lname=?, email=?, password=?, contact=?, gender=? WHERE id=?");
            $stmt->execute([$form['fname'], $form['lname'], $form['email'], password_hash($form['password'], PASSWORD_BCRYPT), $form['contact'], $form['gender'], $id]);
        } else {
            $stmt = $pdo->prepare("UPDATE patreg SET fname=?, lname=?, email=?, contact=?, gender=? WHERE id=?");
            $stmt->execute([$form['fname'], $form['lname'], $form['email'], $form['contact'], $form['gender'], $id]);
        }
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Patient updated successfully!'];
...
<div class="card">
  <div class="card-header">
    <div>
      <div class="card-title" style="color:#0d6efd;"><i class="fa-solid fa-user-pen me-2"></i>Edit Patient Record</div>
      <div class="card-subtitle">Update patient information below.</div>
    </div>
  ...
  <div class="card-body p-4">
    <form method="POST" novalidate>
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <div class="input-group">
            ...
            <input type="text" name="fname" class="form-control <?= isset($errors['fname'])?'is-invalid':'' ?>" value="<?= htmlspecialchars($form['fname']) ?>">
        ...
        <div class="col-md-6">
          ...
          <div class="input-group">
            <span class="input-group-text"><i class="fa-solid fa-user text-muted"></i></span>
            <input type="text" name="lname" class="form-control <?= isset($errors['lname'])?'is-invalid':'' ?>" value="<?= htmlspecialchars($form['lname']) ?>">
        ...
        <div class="col-md-6">
          ...
          <div class="input-group">
            ...
            <input type="text" name="contact" class="form-control <?= isset($errors['contact'])?'is-invalid':'' ?>" value="<?= htmlspecialchars($form['contact']) ?>">
      ...
      <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
        ...
        <button type="submit" class="btn btn-primary px-4"><i class="fa-solid fa-floppy-disk me-1"></i> Update Patient</button>
      </div>
    </form>
  </div>
</div>
